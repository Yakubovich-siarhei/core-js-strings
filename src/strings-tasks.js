...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    const firstChar = str.indexOf(value);
    const res = str.slice(0, firstChar);
    const res2 = str.slice(firstChar + value.length);
    return res + res2;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    const firstChar = str.lastIndexOf(value);
    const res = str.slice(0, firstChar);
    const res2 = str.slice(firstChar + value.length);
    return res + res2;
  }
...
function sumOfCodes(str) {
  if (str && str.length > 0) {
    ...
    let result = arrays.map((item) => item.charCodeAt(0));
...
function formatTime(minutes, seconds) {
  ...
  if (`${minutes}`.length < 2) min += `${minutes}`.padStart(2, '0');
  else min = minutes;
  if (`${seconds}`.length < 2) sec += `${seconds}`.padStart(2, '0');
  else sec = seconds;
  res = `${min}:${sec}`;
...
function countVowels(str) {
  if (str.length > 0) {
    const m = str.toLowerCase().match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  const res = str.toLowerCase().replaceAll(/[^a-zа-яё]/gi, '');
...
function findLongestWord(input) {
  return input.split(/\W+/).reduce((longest, word) => {
...
function reverseWords(/* str */) {
  ...
  // return str.split('').reverse().join('');  this
}
